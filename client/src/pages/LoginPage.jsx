    ...
    sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
        ...
        flex-col gap-6 rounded-lg shadow-lg'>
          ...
          <div className='flex flex-col gap-2 '>
            {
              currState === "Sign up" ?(
                <p className='text-sm text-gray-600'> Already habe an account? 
                <span onClick={()=>{setCurState("Login");setIsDataSubmitted(false)}} className='font-medium text-violet-500 cursor-pointer'> Login here</span></p>
              ):(
                <p className='text-sm text-gray-600 '>Create an account
                 <span onClick={()=>{setCurState("Sign up");}} className='font-medium text-violet-500 cursor-pointer'> Click here</span></p>
              )
