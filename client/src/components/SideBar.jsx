...
const SideBar = ({selectedUser,setSelectedUser}) => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-3 md:p-5 rounded-r-xl text-white
        ...
        <div className='flex flex-col'>
            {userDummyData.map((user,index)=>(
                <div onClick={()=>{setSelectedUser(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id===user._id&&'bg-[#282142]/50'}`}>
                    ...
                    {index>2&&<p className='absolute top4 right-4 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-violet-500/50'>{index}</p>}
                </div>
            ))}
        </div>
    </div>
